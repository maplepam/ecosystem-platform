...
**Consumers** (product apps) normally depend on this repo via **`git:`** `url` + `path` + `ref` and do **not** clone it unless they develop platform code.
...
| **Depend on platform only** (no boilerplate clone) | **[CONTRIBUTING.md — Consuming without the boilerplate](CONTRIBUTING.md#consuming-without-the-boilerplate)** and the same BOM idea: **[platform_bom.yaml](https://github.com/maplepam/ecosystem_boilerplate/blob/main/docs/meta/platform_bom.yaml)** |
...
Work on a branch, open a PR to **`main`** (or your org’s default). See **[CONTRIBUTING.md](CONTRIBUTING.md)** for boundaries, versioning, and consumer impact. After merge, **consumers** bump the **same commit SHA** (or tag) on **every** `emp_ai_*` Git dependency in their host `pubspec.yaml` — see boilerplate **[platform_bom.yaml](https://github.com/maplepam/ecosystem_boilerplate/blob/main/docs/meta/platform_bom.yaml)**.
